...
def translate_and_reconstruct(nets, args, x_src, y_src, x_ref, y_ref, filename):
    ...
    x_fake = nets.generator(x_src, s_ref, x_ref, masks=masks)
    s_src = nets.style_encoder(x_src, y_src)
    masks = nets.fan.get_heatmap(x_fake) if args.w_hpf > 0 else None
    x_rec = nets.generator(x_fake, s_src, x_src, masks=masks)
...
def translate_using_latent(nets, args, x_src, y_trg_list, z_trg_list, psi, filename):
    ...
    for i, y_trg in enumerate(y_trg_list):
        ...
        for z_trg in z_trg_list:
            ...
            noise = torch.zeros(s_trg.size(0), 3, 256, 256)
            x_fake = nets.generator(x_src, s_trg, noise, masks=masks)
            x_concat += [x_fake]
...
#     '''
#     x_concat = torch.cat(x_concat, dim=0)
#     save_image(x_concat[12], N, filename)
#     img_fake = x_concat[12].unsqueeze(0)
#     fake_HF = HF(img_fake)[1].squeeze(0)
#     fake_HF = toPIL(fake_HF)
#     fake_HF.save(filename_HF)
#     '''
...
#
# @torch.no_grad()
# def save_wave_images(x_src, filename):
#     LL_real, LH_real, HL_real, HH_real = Pool(x_src)
#     print(LL_real.size())
#
...
#         img_real.save(os.path.join(filename, 'img_real.jpg'))
#
#         realLL = LL_real[i, :, :, :]
#         img_realLL = toPIL(realLL)
#         img_realLL.save(os.path.join(filename, 'LL_real.jpg'))
#
#         realLH = LH_real[i, :, :, :]
#         img_realLH = toPIL(realLH)
#         img_realLH.save(os.path.join(filename, 'LH_real.jpg'))
#
#         realHL = HL_real[i, :, :, :]
#         img_realHL = toPIL(realHL)
#         img_realHL.save(os.path.join(filename, 'HL_real.jpg'))
#         realHH = HH_real[i, :, :, :]
#         img_realHH = toPIL(realHH)
#         img_realHH.save(os.path.join(filename, 'HH_real.jpg'))
#
#         realHF = realLH + realHL + realLL
#         img_realHF = toPIL(realHF)
#         img_realHF.save(os.path.join(filename, 'HF_real.jpg'))
...
def interpolate(nets, args, x_src, s_prev, s_next):
    ...
    for alpha in alphas:
        ...
        x_fake = nets.generator(x_src, s_ref, x_src, masks=masks)
