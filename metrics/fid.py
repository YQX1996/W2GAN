...
def calculate_kid_given_paths(paths, img_size=256, batch_size=50):
    """Calculates the KID of two paths"""
    print('Calculating KID given paths %s and %s...' % (paths[0], paths[1]))
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    inception = InceptionV3().eval().to(device)
    loaders = [get_eval_loader(path, img_size, batch_size) for path in paths]

    real_fature = fid.get_folder_features(paths[0])
    fake_fature = fid.get_folder_features(paths[1])

    kid = fid.kernel_distance(real_fature, fake_fature)
