...
from metrics.fid import calculate_fid_given_paths, calculate_kid_given_paths
...
def calculate_metrics(nets, args, step, mode):
            ...
            for i, x_src in enumerate(tqdm(loader_src, total=len(loader_src))):
                ...
                for j in range(args.num_outs_per_domain):
                    if mode == 'latent':
                        ...
                        noise = torch.zeros(s_trg.size(0), 3, 256, 256).cuda()
                        ...
                        x_fake = nets.generator(x_src, s_trg, x_ref, masks=masks)
    ...
    # calculate and report Kid values
    calculate_kid_for_all_tasks(args, domains, step=step, mode=mode)
...
def calculate_kid_for_all_tasks(args, domains, step, mode):
    print('Calculating KID for all tasks...')
    kid_values = OrderedDict()
    for trg_domain in domains:
        src_domains = [x for x in domains if x != trg_domain]
        ...
        for src_domain in src_domains:
            ...
            path_real = os.path.join(args.val_img_dir, trg_domain)
            path_fake = os.path.join(args.eval_dir, task)
            print('Calculating KID for %s...' % task)
            kid_value = calculate_kid_given_paths(
                paths=[path_real, path_fake],
                img_size=args.img_size,
                batch_size=args.val_batch_size)
            kid_values['kID_%s/%s' % (mode, task)] = kid_value

    # calculate the average KID for all tasks
    kid_mean = 0
    for _, value in kid_values.items():
        kid_mean += value / len(kid_values)
    kid_values['KID_%s/mean' % mode] = kid_mean

    # report KID values
    filename = os.path.join(args.eval_dir, 'KID_%.5i_%s.json' % (step, mode))
    utils.save_json(kid_values, filename)